...
def process(args):
    ...
    ### clean the document
    assert "text" in list(data.columns), "data format error,there is no text colunm in this corpus"
    if  args.lang=="en":
    ...
    cv = CountVectorizer(strip_accents = 'unicode',#将使用unicode编码在预处理步骤去除raw document中的重音符号
                                    ...
                                    min_df = 10 # 如果某个词的document frequence小于min_df，则这个词不会被当作关键词
                                )
    ...
    with open(f"{args.output_path}{bow_path}",'wb') as fw:
        pickle.dump(tf,fw)
    ######### save the entity ###########
    #### save document`s relationshop between other entity
    ### journal 
    ...
    docuemnt_word_df = pd.DataFrame(data=document_word_relation,columns=["document_id","word_id","frequency"]) # 3037434
    docuemnt_word_df.to_csv(f"{args.output_path}/document_word.csv",index=False)
    #### journal map 作者
    if len(list(set(["authors","journal"]) & set(data.columns))) == 2:
        ...
        authors_journal_df = pd.DataFrame(data = authors_journal_relation,columns=["document_id","author_id","journal_id"])
    ...
    word_word_df = pd.DataFrame(data=word_word_occur_relationship,columns=["word_id","aim_word_id","value","relationship_type"]) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', type=str, default = "./covid-2022-11-input/")
